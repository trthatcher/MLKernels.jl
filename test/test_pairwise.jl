...
for kernelobject in (
    ...
    @test MLKernels.pairwise(ARD(k,w[1:1]), x1[1], y1[1]) == w[1]^2*MLKernels.kappa(k, x1[1], y1[1])
    print(" Vector")
    @test MLKernels.pairwise(k, x1, y1) == MLKernels.kappa(k, x1[1], y1[1]) + MLKernels.kappa(k, x1[2], y1[2])
    @test MLKernels.pairwise(ARD(k,w), x1, y1) == w[1]^2*MLKernels.kappa(k, x1[1], y1[1]) + w[2]^2*MLKernels.kappa(k, x1[2], y1[2])
    print(" Matrix")
