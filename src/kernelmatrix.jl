...
#   If trans == 'N' -> K = ϕ(X)ϕ(X)ᵀ (X is a design matrix IE rows are coordinates)
#            == 'T' -> K = ϕ(X)ᵀϕ(X) (X is a transposed design matrix IE columns are coordinates)
...
function kernelmatrix_scaled{T<:FloatingPoint}(a::T, κ::StandardKernel{T}, X::Matrix{T}, trans::Char = 'N', uplo::Char = 'U', sym::Bool = true)
...
function kernelmatrix_product{T<:FloatingPoint}(a::T, κ₁::StandardKernel{T}, κ₂::StandardKernel{T}, X::Matrix{T}, trans::Char = 'N', uplo::Char = 'U', sym::Bool = true)
...
function kernelmatrix_sum{T<:FloatingPoint}(a₁::T, κ₁::StandardKernel{T}, a₂::T, κ₂::StandardKernel{T}, X::Matrix{T}, trans::Char = 'N', uplo::Char = 'U', sym::Bool = true)
    K = kernelmatrix_scaled(a₁, κ₁, X, trans, uplo, sym)
    BLAS.axpy!(length(K), a₂, kernelmatrix(κ₂, X, trans, uplo, sym), 1, K, 1)
end

function kernelmatrix{T<:FloatingPoint}(ψ::ScaledKernel{T}, X::Matrix{T}, trans::Char = 'N', uplo::Char = 'U', sym::Bool = true)
    kernelmatrix_scaled(ψ.a, ψ.k, X, trans, uplo, sym)
end

function kernelmatrix{T<:FloatingPoint}(ψ::KernelProduct{T}, X::Matrix{T}, trans::Char = 'N', uplo::Char = 'U', sym::Bool = true)
    kernelmatrix_product(ψ.a, ψ.k1, ψ.k2, X, trans, uplo, sym)
end

function kernelmatrix{T<:FloatingPoint}(ψ::KernelSum{T}, X::Matrix{T}, trans::Char = 'N', uplo::Char = 'U', sym::Bool = true)
...
#   trans == 'N' -> K = ϕ(X)ϕ(Y)ᵀ (X and Y are design matrices)
#         == 'T' -> K = ϕ(X)ᵀϕ(Y) (X and Y are transposed design matrices)
...
function kernelmatrix_scaled{T<:FloatingPoint}(a::T, κ::StandardKernel{T}, X::Matrix{T}, Y::Matrix{T}, trans::Char = 'N')
    K = kernelmatrix(κ, X, Y, trans)
    a == one(T) ? K : BLAS.scal!(length(K), a, K, 1)
end

function kernelmatrix_product{T<:FloatingPoint}(a::T, κ₁::StandardKernel{T}, κ₂::StandardKernel{T}, X::Matrix{T}, Y::Matrix{T}, trans::Char = 'N')
    K = kernelmatrix_scaled(a, κ₁, X, Y, trans)
    hadamard!(K, kernelmatrix(κ₂, X, Y, trans))
end

function kernelmatrix_sum{T<:FloatingPoint}(a₁::T, κ₁::StandardKernel{T}, a₂::T, κ₂::StandardKernel{T}, X::Matrix{T}, Y::Matrix{T}, trans::Char = 'N')
    K = kernelmatrix_scaled(a₁, κ₁, X, Y, trans)
    BLAS.axpy!(length(K), a₂, kernelmatrix(κ₂, X, Y, trans), 1, K, 1)
end

function kernelmatrix{T<:FloatingPoint}(ψ::ScaledKernel{T}, X::Matrix{T}, Y::Matrix{T}, trans::Char = 'N')
    kernelmatrix_scaled(ψ.a, ψ.k, X, Y, trans)
end
function kernelmatrix{T<:FloatingPoint}(ψ::KernelProduct{T}, X::Matrix{T}, Y::Matrix{T}, trans::Char = 'N')
    kernelmatrix_product(ψ.a, ψ.k1, ψ.k2, X, Y, trans)
end
function kernelmatrix{T<:FloatingPoint}(ψ::KernelSum{T}, X::Matrix{T}, Y::Matrix{T}, trans::Char = 'N')
...
for (kernelobject, gramian) in ((:SquaredDistanceKernel, :sqdistmatrix),
    ...
    function kappa_gramian!{T<:FloatingPoint}(κ::$kernelobject{T}, G::Array{T}, uplo::Char, sym::Bool = true)
